...
- **`.env` injection on create** — when a session has a `working_dir`, `create_session` writes a `.env` there with `MY_SESSION_ID` (always) and `PARENT_SESSION_ID` (when spawned), appended after any operator/inherited env so the injected keys win.
