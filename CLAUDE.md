...
- **`subagent` tool** — calls `POST /api/v1/sessions` on the gateway to spawn a child, passing the caller's id as `parent_session_id`, then sends the task as the first message. Returns immediately with the child's id (fire-and-forget). `isolation="worktree"` creates a real git worktree off the parent's working dir on branch `subagent/<id>` (falls back to an empty scratch dir under `AUGENTIA_WORKTREE_ROOT` / `backend/tmp/sessions` if the parent isn't a git repo). No `.env` is copied into the child — it inherits the backend process's environment like any other session.
